...
@dataclass
class ModelConfig:
    # Probability of dropping the class label in flow-matching
    class_dropout_prob: float = 0.1
    # Integer representing the 'null' class for dropped labels
...
def get_targets(FLAGS, key, train_state, images, labels, force_t=-1, force_dt=-1):
    ...
    if FLAGS.model['bootstrap_dt_bias'] == 0:
        ...
        dt_base = jnp.concatenate(
            [
                dt_base,
                jnp.zeros(
                    bootstrap_batchsize - dt_base.shape[0],
                ),
            ]
        )
        ...
        dt_base = jnp.repeat(
            log2_sections - 1 - jnp.arange(log2_sections - 2), (bootstrap_batchsize // 2) // log2_sections
        )
    ...
    # Sample t.
    t = jax.random.randint(time_key, (images.shape[0],), minval=0, maxval=FLAGS.model['denoise_timesteps']).astype(
        jnp.float32
    )
